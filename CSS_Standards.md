...
For lines needing indenting, use 2 spaces for each level of indentation, the same standard as Drupal’s PHP and JavaScript code.
...
#Positioning properties include: position, float, clear, top, right, bottom, left, direction, and z-index.
#Box model properties include: display, [(max|min)-]height, [(max|min)-]width, margin, padding, border and their various longhand forms (margin-top, etc.) Plus box-sizing.
#Other declarations.
